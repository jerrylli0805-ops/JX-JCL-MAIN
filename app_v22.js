...
// ==========================================
// --- OFFLINE ICON SYSTEM (內建離線圖示庫) ---
// ==========================================

// 1. 定義圖示的路徑資料 (SVG Path)
// 這裡收錄了您系統中用到的所有圖示，直接內建，不用上網抓
const OFFLINE_ICONS = {
...
// 2. 改寫「安全渲染函式」：不依賴 Lucide 函式庫，直接用內建資料取代
function safeRenderIcons() {
    // 抓取所有還沒轉換的 <i data-lucide="..."> 標籤
    ...
    elements.forEach(element => {
        ...
        if (svgContent) {
            ...
            // 繼承原本 i 標籤的 class (例如 w-4 h-4 text-white...)
            ...
            console.warn(`Icon not found in offline library: ${iconName}`);
        }
    });
}
...
const MENUS = {
    ...
    'TPE-KMJ': {
        meal_1: [
            { 
                code: 'MBF1', 
                name: 'Japanese Style Braised Beef Rib', 
                chinese: '和風大根燉牛肋條', 
                dessert: 'ONE TRAY SERVICE' // 修改這裡
            },
            { 
                code: 'MPK1', 
                name: 'Herb Roasted Pork Shoulder Chop', 
                chinese: '香料烤豬肩排・義式寬麵', 
                dessert: 'ONE TRAY SERVICE' // 修改這裡
            },
            { 
                code: 'MFI1', 
                name: 'Grilled Halibut w/ Truffle Potato', 
                chinese: '炙烤比目魚・松露洋芋', 
                dessert: 'ONE TRAY SERVICE' // 修改這裡
            },
        ],
...
const ROUTES = [
    ...
    { id: 'TPE-KMJ', name: 'Taipei - FUK/KMJ (SS)', type: 'SS' },
    { id: 'HKG-TPE', name: 'Hong Kong - Taipei (SS)', type: 'SS' },
    { id: 'KMJ-TPE', name: 'Kumamoto - Taipei (SS)', type: 'SS' },
    // --- S (Short) 短程 ---
...
// [FIXED] setupInventoryInputs - 調整執行順序與加入防呆
...
// [NEW] 處理完全重置 (Restart)
function handleFinalRestart() {
    if (!confirm('This will clear all data and return to the start screen. Continue?')) return;
    ...
    // 關閉 Modal (如果 resetSystem 沒有 reload 頁面的話)
    const modal = document.getElementById('end-flight-modal');
    if (modal) {
