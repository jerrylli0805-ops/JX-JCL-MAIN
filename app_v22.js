...
// [CRITICAL FIX] renderGalleyDashboard - 分離「總需求」與「剩餘庫存」計算邏輯
function renderGalleyDashboard() {
    // 定義兩種計數器：
    // 1. Total: 用於顯示 ORDERED (包含預選，讓廚房知道總共要熱多少)
    // 2. Field: 用於計算 LEFT (只算現場點餐，用於扣除一般庫存)
    ...
    orders.forEach(o => {
        ...
        if (['ORDERED', 'DELAYED', 'DND'].includes(o.status)) {
            ...
            if (o.appetizerChoice && o.appetizerChoice !== 'Standard') {
                // A. 總需求
                totalCountsApp[o.appetizerChoice] = (totalCountsApp[o.appetizerChoice] || 0) + 1;
                
                // B. 現場數 (排除預選)
                if (!o.isAppetizerPreSelect) {
                    fieldCountsApp[o.appetizerChoice] = (fieldCountsApp[o.appetizerChoice] || 0) + 1;
                }
            }
        }
    });
    ...
    // --- 卡片生成器 (核心邏輯修改) ---
    // orderedTotal: 總需求 (顯示在 Ordered)
    // orderedField: 現場消耗 (用於計算 Left)
    const createInventoryCard = (code, orderedTotal, orderedField, name, colorClass, inventoryMap) => {
        // 取得總裝載量 (Inventory Load)
        const totalLoaded = (inventoryMap && inventoryMap[code]) ? inventoryMap[code] : 0;
        
        // [關鍵修正] 剩餘量 = 總裝載 (Load) - 現場消耗 (Field Orders)
        // 預選的不會扣這裡，因為預選是額外上載的
        ...
        // 進度條 (視覺化 Field Order 對 Load 的消耗)
            ...
            <div class="bg-gray-800 border border-gray-700 rounded-2xl overflow-hidden shadow-lg relative group">
                ...
                <div class="p-4">
                    <div class="flex justify-between items-start mb-3">
                        ...
                        <span class="text-[10px] font-mono bg-gray-700 text-gray-300 px-2 py-1 rounded" title="General Inventory Load">
                            LOAD: ${totalLoaded}
                        </span>
                    </div>

                    <div class="flex items-end justify-between border-t border-gray-700 pt-3">
                        
                        <div class="text-center" title="Total pax eating this (Pre-select + Field)">
                        ...
                        <div class="text-center" title="Remaining Inventory (Load - Field Orders)">
    ...
    activeMeals_1.forEach(m => {
        ...
        gridM1.innerHTML += createInventoryCard(m.code, total, field, m.name, 'bg-amber-500', mealInventory_1);
    });
    ...
    if (activeMeals_2.length > 0) {
        ...
        activeMeals_2.forEach(m => {
            ...
            gridM2.innerHTML += createInventoryCard(m.code, total, field, m.name, 'bg-blue-500', mealInventory_2);
        });
    } else {
    ...
    if (activeAppetizers.length > 0) {
        sectionApp.classList.remove('hidden');
        activeAppetizers.forEach(app => {
            const total = totalCountsApp[app.code] || 0;
            const field = fieldCountsApp[app.code] || 0;
            gridApp.innerHTML += createInventoryCard(app.code, total, field, app.name, 'bg-green-500', appetizerInventory);
        });
    } else {
    ...
    if (Object.keys(countsSPML).length > 0) {
        for (const [code, count] of Object.entries(countsSPML)) {
             gridSPML.innerHTML += `
                <div class="bg-gray-800 border border-purple-500/50 rounded-xl p-3 text-center">
                    <span class="text-3xl font-bold text-white block">${count}</span>
                    <span class="text-sm font-bold text-purple-400">${code}</span>
                </div>
            `;
        }
    } else {
...
// [最終完整版] renderSeatLayout: 整合 Delay 計時、取消顯示、嬰兒過敏圖示
function renderSeatLayout() {
    ...
    // Helper: 產生服務按鈕的樣式 (取消的變紅/刪除線，正常的保持原樣)
    const getServiceBtnClass = (served, skipped, cancelled) => {
        if (cancelled) return 'service-item-btn text-red-400 line-through opacity-70 border-red-900/30 bg-red-900/10'; 
        return 'service-item-btn';
    };
    ...
    activeAircraftConfig.rows.forEach(row => {
        ...
        activeAircraftConfig.seatLetters.forEach(letter => {
            ...
            if (order.serviceClosed) {
                bgClass = 'bg-gray-800 opacity-60 border-gray-700';
            }
                ...
                // 如果 delayUntil 是 0 或不存在，代表還沒進入 Service Mode，顯示等待中
                if (!order.delayUntil || order.delayUntil === 0) {
                    bgClass = 'bg-blue-700/80 border-blue-500';
                    delayDisplayHtml = `
                        <div class="mt-1 bg-blue-900/50 text-blue-200 text-sm font-mono py-1 px-2 rounded border border-blue-400/30 flex items-center justify-center gap-2">
                    ...
                    // 計算剩餘時間
                    const now = Date.now();
                    const diffMs = order.delayUntil - now;
                    ...
                    if (diffMs > 0) {
                        ...
                        delayDisplayHtml = `
                            <div class="mt-1 bg-blue-900/50 text-blue-200 text-sm font-mono py-1 px-2 rounded border border-blue-400/30 flex items-center justify-center gap-2">
                                ...
                                <span>${diffMins} min left</span>
                            </div>`;
                    } else {
                        // 時間到 (Overdue)
                        bgClass = 'bg-amber-600 border-amber-400 animate-pulse ring-4 ring-amber-500/50';
                        delayDisplayHtml = `
                            <div class="mt-1 bg-red-900 text-white text-sm font-bold py-1 px-2 rounded border border-red-500 animate-bounce">
                                ⚠️ SERVICE DUE
                            </div>`;
                        
                        // 觸發鈴聲與通知 (防止重複觸發)
                        if (!order.notificationShown && currentMode === MODES.SERVICE_MODE) {
                            playNotificationSound(); // 呼叫音效
                            showDueServiceAlert(seatId); // 呼叫彈窗
            ...
            let infantHtml = '';
            if (order.hasInfant) {
                infantHtml = `<span class="ml-2 inline-flex items-center justify-center bg-pink-500 text-white rounded-full p-1 shadow-sm"><i data-lucide="baby" class="w-5 h-5"></i></span>`;
            }
            header.innerHTML = `<div class="flex items-center"><p class="text-3xl font-extrabold text-amber-300">${seatId}</p>${infantHtml}</div>`;
                ...
                order.tags.forEach(tagId => {
                    ...
                    if (tag) {
                        if (tagId === 'allergic' && order.allergyNote) {
                            tagsDiv.innerHTML += `
                                <div class="flex items-center bg-red-100 border border-red-400 rounded px-1.5 py-0.5">
                                    <i data-lucide="${tag.icon}" class="w-4 h-4 text-red-600 mr-1"></i>
                                    <span class="text-[10px] font-bold text-red-700 uppercase leading-none max-w-[60px] truncate">${order.allergyNote}</span>
                                </div>`;
                        } else {
                            tagsDiv.innerHTML += `<i data-lucide="${tag.icon}" class="w-5 h-5 ${tag.color}"></i>`;
                        }
                    }
                });
            }
            ...
            if (delayDisplayHtml) {
                ...
                timerDiv.innerHTML = delayDisplayHtml;
                seatCard.appendChild(timerDiv);
            }
            ...
            if (order.status === 'ORDERED' && !order.serviceClosed) {
                ...
                if (currentServicePhase === 'MEAL_1') {
                    ...
                    if (showStarter && ((!order.appetizerServed && !order.appetizerSkipped) || order.appetizerCancelled)) {
                        ...
                        btn.innerHTML = `<span>Starter: ${order.appetizerChoice || 'Standard'} ${order.appetizerCancelled ? '(CNX)' : ''}</span>`;
                    ...
                    if (isLongHaul && ((!order.soupServed && !order.soupSkipped) || order.soupCancelled)) {
                        ...
                        btn.innerHTML = `<span>Soup ${order.soupCancelled ? '(CNX)' : ''}</span>`;
                    ...
                    if (order.mealCode && ((!order.mealServed && !order.mealSkipped) || order.mealCancelled)) {
                        const btn = document.createElement('div');
                        btn.className = getServiceBtnClass(false, false, order.mealCancelled);
                        btn.innerHTML = `<span>Main: ${order.mealCode} ${order.mealCancelled ? '(CNX)' : ''}</span>`;
                    ...
                    if (order.isSPML) { if (!isSSRoute) showDessertBtn = true; } 
                        ...
                        if (mData && mData.dessert && mData.dessert !== 'ONE TRAY SERVICE') {
                            showDessertBtn = true; dessertName = mData.dessert;
                        }
                    }
                    if (showDessertBtn && ((!order.dessertServed && !order.dessertSkipped) || order.dessertCancelled)) {
                        ...
                        btn.innerHTML = `<span>${dessertName} ${order.dessertCancelled ? '(CNX)' : ''}</span>`;
                        btn.onclick = e => { e.stopPropagation(); handleServiceClick(e, seatId, 'dessert'); };
                        itemsContainer.appendChild(btn);
                    }
                } else {
                    // Meal 2 (Yogurt/Main/Fruit)
                    ...
                    if (isLongHaul) {
                        ...
                        if (!isCongee && ((!order.yogurtServed && !order.yogurtSkipped) || order.yogurtCancelled)) {
                            ...
                            btn.innerHTML = `<span>Yogurt ${order.yogurtCancelled ? '(CNX)' : ''}</span>`;
                        ...
                        if (order.mealCode_2 && ((!order.mealServed_2 && !order.mealSkipped_2) || order.mealCancelled_2)) {
                            const btn = document.createElement('div');
                            btn.className = getServiceBtnClass(false, false, order.mealCancelled_2);
                            btn.innerHTML = `<span>M2: ${order.mealCode_2} ${order.mealCancelled_2 ? '(CNX)' : ''}</span>`;
                        ...
                        if ((!order.fruitServed && !order.fruitSkipped) || order.fruitCancelled) {
                            ...
                            btn.innerHTML = `<span>Fruit ${order.fruitCancelled ? '(CNX)' : ''}</span>`;
                ...
                if (currentBevs && currentBevs.length > 0) {
                    currentBevs.forEach((bev, idx) => {
                        // 顯示條件：(!Served AND !Skipped) OR Cancelled
                        if ((!bev.served && !bev.skipped) || bev.cancelled) {
                            const btn = document.createElement('div');
                            // 套用取消樣式
                            btn.className = `flex items-center justify-center gap-1 ${getServiceBtnClass(false, false, bev.cancelled)}`;
                            
                            const styleText = bev.style ? ` (${bev.style})` : '';
                            const cnxText = bev.cancelled ? ' (X)' : ''; // 簡短標示
                            
                            btn.innerHTML = `<i data-lucide="coffee" class="w-3 h-3"></i> ${bev.name}${styleText}${cnxText}`;
                            
                            const targetType = (currentServicePhase === 'MEAL_1') ? 'drink_1' : 'drink_2';
                            btn.onclick = e => { 
                                e.stopPropagation(); handleServiceClick(e, seatId, targetType, idx);
                            };
                            itemsContainer.appendChild(btn);
                        }
                    });
            ...
            if (currentBevs) {
                currentBevs.forEach(bev => {
                    // 只顯示已完成的 (Cancelled 已經在上面按鈕區顯示了)
                    if (bev.served || bev.skipped) {
                        const styleText = bev.style ? ` <span class="text-[10px] opacity-70">[${bev.style}]</span>` : '';
                        summaryDiv.innerHTML += `<div class="line-through text-gray-400 italic">✓ ${bev.name}${styleText}</div>`;
                    }
                });
            }
            ...
            // Meal 2 Summary items
            if (order.yogurtServed) summaryDiv.innerHTML += `<div class="text-gray-400 text-xs">✓ Yogurt Done</div>`;
            if (order.mealServed_2) summaryDiv.innerHTML += `<div class="text-gray-400 text-xs font-bold">✓ M2: ${order.mealCode_2}</div>`;
            if (order.fruitServed) summaryDiv.innerHTML += `<div class="text-gray-400 text-xs">✓ Fruit Done</div>`;
...
// [最終還原版] renderOrderSummaryAggregate - 恢復左右雙欄與飲料分類顯示
// [最終功能版] renderOrderSummaryAggregate - 支援點擊打勾 (備餐確認)
function renderOrderSummaryAggregate() {
    ...
    let leftPanelHtml = `
        <div class="bg-gray-800 p-5 rounded-xl border border-gray-700 shadow-lg h-full">
            <h3 class="text-amber-400 font-bold text-lg mb-4 border-l-4 border-amber-500 pl-3">
    ...
    let rightPanelHtml = `
        <div class="bg-gray-800 p-5 rounded-xl border border-gray-700 shadow-lg h-full">
            ...
            <h4 class="text-white font-bold text-sm mb-4 border-b border-gray-600 pb-2">
                ${mealPhaseTitle} Beverages
            </h4>
            <div class="space-y-5 custom-scrollbar overflow-y-auto max-h-[400px] pr-2">`;
    ...
    if (Object.keys(bevCounts).length === 0) {
        ...
        categoryOrder.forEach(catName => {
            ...
            if (items) {
                rightPanelHtml += `
                    <div>
                        <h5 class="text-[11px] font-bold text-blue-400 uppercase tracking-wider mb-2">${catName}</h5>
...
function setupBeverageOptions(container, selectedBeverages = [], checkboxName = 'beverage') {
    ...
    // 1. 定義各類型的選項內容
    const STYLE_MAP = {
        'ice': ['Normal Ice (正常冰)', 'Less Ice (少冰)', 'No Ice (去冰)', 'Warm (溫)'],
        'temp': ['Hot (熱)', 'Iced (冰)'],
        'whisky': ['Neat (純飲)', 'On the Rocks (加冰)', 'Water Splash (加水)'], // 您提到的 On the Rock
        'soda_ice': ['Normal Ice (正常冰)', 'Less Ice (少冰)', 'No Ice (去冰)'],
        'juice': ['Normal Ice (正常冰)', 'Less Ice (少冰)', 'No Ice (去冰)']
    };
...
async function handleEndOfFlight() {
    if (!confirm('Are you sure you want to end this flight? This will clear the auto-save cache.')) return; // 建議加個防呆

    showMessage('Generating PDF report...', false);
    
    // 清除 LocalStorage
    localStorage.removeItem(STORAGE_KEY); // <--- [關鍵] 清除備份，因為航班已結束
    try {
        generatePDFReport();
    } catch(e) {
