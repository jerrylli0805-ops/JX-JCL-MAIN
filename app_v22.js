...
// [修正版] renderSeatLayout - 修復 A321neo 左右側顯示重疊問題
    ...
    activeAircraftConfig.rows.forEach(row => {
        ...
        activeAircraftConfig.seatLetters.forEach(letter => {
            ...
            if (filter === 'ALL') {
                ...
                let targetCol = 1; // 預設放左邊 (第1欄)
                
                // [FIX] 針對 A321neo 的特殊判斷
                if (currentAircraftType === 'A321neo') {
                    // A321 L-Side: A (第1欄), C (第2欄)
                    if (filter === 'L' && letter === 'C') targetCol = 2;
                    
                    // A321 R-Side: H (第1欄), K (第2欄)
                    if (filter === 'R' && letter === 'K') targetCol = 2;
                } else {
                    // 廣體機 (A330/A350) 的判斷邏輯保持不變
                    if (filter === 'L') {
                        if (['D', 'E'].includes(letter)) targetCol = 2;
                    } else if (filter === 'R') {
                        if (['H', 'K'].includes(letter)) targetCol = 2;
                    }
                }
            ...
            // (以下內容保持不變: Header, Name, Buttons, Summary...)
            ...
            if (order.status === 'ORDERED' && !order.serviceClosed) {
                if (currentServicePhase === 'MEAL_1') {
                    ...
                    if (order.isSPML) {
                        if (!isSSRoute) showDessertBtn = true; 
                    } else {
