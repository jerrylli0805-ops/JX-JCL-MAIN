...
function renderOrderSummaryAggregate() {
    ...
    let rightPanelHtml = `
        <div class="bg-gray-800 p-5 rounded-xl border border-gray-700 shadow-lg h-full flex flex-col">
            <h3 class="text-amber-400 font-bold text-lg mb-4 border-l-4 border-amber-500 pl-3 shrink-0">
                Beverages Summary
            </h3>
            <h4 class="text-white font-bold text-sm mb-4 border-b border-gray-600 pb-2 shrink-0">
...
// [修正版 - Dark Theme] renderPreSelectInputs
// [修正版 - White & Gold Theme] renderPreSelectInputs
// [修正版 - White & Gold Theme with Dark Blue SPML Btn] renderPreSelectInputs
function renderPreSelectInputs() {
    ...
    seatIds.forEach(seatId => {
        ...
        div.innerHTML = `
            ...
            <input type="text" id="preselect-name-${seatId}" class="${selectClass} placeholder-gray-300 font-bold uppercase tracking-wide" placeholder="LAST NAME">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (inventoryModal) {
        ...
        inventoryModal.classList.remove('flex'); // 確保移除 flex，避免破版
    }
    if (appContainer) {
        appContainer.classList.add('hidden');
    }
    ...
    const enterSystem = () => {
        ...
        if (hasBackup) {
            // 如果有備份，init() 裡的 checkAndRestoreState 會處理
            // 我們這裡不需要強制開視窗，以免蓋住還原的提示
        } else {
            // 如果沒有備份 (代表是新航班)，強制顯示設定視窗！
            if (inventoryModal) {
                inventoryModal.classList.remove('hidden');
                inventoryModal.classList.add('flex'); // 加上 flex 讓它置中顯示
            }
        }
