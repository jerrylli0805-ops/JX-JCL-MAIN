        ...
        activeAircraftConfig.seatLetters.forEach(letter => {
            ...
            if (order.status === 'ORDERED' && !order.serviceClosed) {
                if (currentServicePhase === 'MEAL_1') {
                    ...
                    // --- [FIX 1] 補回甜點按鈕 (Dessert Button) ---
                    // 如果有設定甜點，且還沒服務/跳過，就顯示按鈕
                    const mData = getMeal1(order.mealCode);
                    if (mData && mData.dessert && mData.dessert !== 'ONE TRAY SERVICE' && !order.dessertServed && !order.dessertSkipped) {
                        const btn = document.createElement('div');
                        btn.className = 'service-item-btn';
                        btn.innerHTML = `<span>Dessert: ${mData.dessert}</span>`;
            ...
            // --- [FIX 2] 補回甜點完成狀態 (Dessert Summary) ---
            if (order.dessertServed) {
                const mData = getMeal1(order.mealCode);
                const dName = mData ? mData.dessert : 'Dessert';
                // 排除 SS 航線已經合併在 Main 裡面的情況，避免重複顯示 (但顯示也無妨，這裡我讓它顯示)
...
// [最終修正版 v6] openOrderModal - 修復 Close Service 按鈕消失的問題 (放寬甜點檢查邏輯)
function openOrderModal(seatId) {
    ...
    // 檢查甜點：只有在「有甜點按鈕可按」的情況下，才強制要求甜點狀態
    // 1. 取得餐點資訊
    const meal1 = getMeal1(order.mealCode);
    // 2. 判斷是否有獨立甜點 (One Tray 或 SPML 通常沒有獨立按鈕)
    const hasSeparateDessertButton = meal1 && meal1.dessert && meal1.dessert !== 'ONE TRAY SERVICE';
    // 3. 判定甜點是否完成：(已送出 或 已跳過) 或者 (根本不需要送)
    ...
    if (!isFullOrderMode && isServiceComplete) {
        ...
        // Case 2: Service Mode (In Progress)
        show(appElements.orderPhaseTabs, false);
        show(appElements.orderTabContent1, true);
        show(appElements.orderTabContent2, false);
        appElements.orderTabContent1.innerHTML = renderServiceSummary(order);
