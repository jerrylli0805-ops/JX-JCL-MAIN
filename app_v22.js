...
function renderSeatLayout() {
    ...
    const getServiceBtnClass = (served, skipped, cancelled) => {
        ...
        // [修改點 1] 顏色更亮！
        // bg-[#4a7a70]: 亮湖水綠
        // border-[#74a397]: 淺青瓷色邊框
        return 'w-full py-2 px-3 mb-1.5 rounded-lg border border-[#74a397] bg-[#4a7a70] hover:bg-[#5b8c82] hover:border-[#fbbf24] text-white text-sm font-bold transition-all shadow-md active:scale-95';
    };
    ...
    activeAircraftConfig.rows.forEach(row => {
        ...
        activeAircraftConfig.seatLetters.forEach(letter => {
            ...
            if (order.serviceClosed) {
                ...
                dndDisplayHtml = `
                    <div class="mt-2 bg-red-900/20 text-red-400 text-xs font-mono py-1 px-2 rounded border border-red-800/50 flex items-center justify-center gap-2">
                        <i data-lucide="bell-off" class="w-3 h-3"></i> 
                        <span class="font-bold tracking-wider">DND</span>
                    </div>`;
            }
                ...
                if (!order.delayUntil || order.delayUntil === 0) {
                    ...
                    delayDisplayHtml = `
                        <div class="mt-2 bg-blue-900/20 text-blue-300 text-xs font-mono py-1 px-2 rounded border border-blue-500/30 flex items-center justify-center gap-2">
                            <i data-lucide="clock" class="w-3 h-3"></i> <span>Waiting</span>
                        </div>`;
                } else {
                    const now = Date.now();
                    ...
                    if (diffMs > 0) {
                        ...
                        delayDisplayHtml = `
                            <div class="mt-2 bg-blue-900/30 text-blue-200 text-xs font-mono py-1 px-2 rounded border border-blue-400/50 flex items-center justify-center gap-2">
                                <i data-lucide="timer" class="w-3 h-3 animate-pulse"></i> <span>${diffMins}m left</span>
                            </div>`;
                    } else {
                        bgClass = 'bg-[#2a1a10] border-amber-500 animate-pulse ring-2 ring-amber-500/50';
                        delayDisplayHtml = `
                            <div class="mt-2 bg-amber-900 text-white text-xs font-bold py-1 px-2 rounded border border-amber-500 animate-bounce">
                                ⚠️ SERVICE DUE
                            </div>`;
                        
                        if (!order.notificationShown && currentMode === MODES.SERVICE_MODE) {
                            playNotificationSound(); 
                            showDueServiceAlert(seatId); 
            ...
            seatCard.className = `seat-card p-4 rounded-xl text-center border transition-all duration-300 flex flex-col gap-2 ${bgClass} ${shadowClass}`;
                ...
                order.tags.forEach(tagId => {
                    ...
                    if (tag) {
                        if (tagId === 'allergic' && order.allergyNote) {
                            tagsDiv.innerHTML += `
                                <div class="flex items-center bg-red-900/50 border border-red-700/50 rounded px-1.5 py-0.5">
                                    <i data-lucide="${tag.icon}" class="w-3 h-3 text-red-400 mr-1"></i>
                                    <span class="text-[9px] font-bold text-red-300 uppercase max-w-[50px] truncate">${order.allergyNote}</span>
                                </div>`;
                        } else {
            ...
            if (order.remark) {
                seatCard.innerHTML += `
                    <div class="text-left bg-amber-900/20 border border-amber-700/30 rounded p-1.5 mb-2 flex items-start gap-1.5">
                        <i data-lucide="sticky-note" class="w-3 h-3 text-amber-500 mt-0.5 shrink-0"></i>
                        <span class="text-xs text-amber-200/90 italic leading-tight break-words">${order.remark}</span>
                    </div>`;
            }
            ...
            if (order.status === 'ORDERED' && !order.serviceClosed) {
                ...
                // [修改點 2] 統一按鈕內容結構：強制 Flex 置中 + 微調間距
                const createBtnContent = (iconName, text, subText = '', cnx = false) => {
                    const cnxText = cnx ? ' (CNX)' : '';
                    return `
                        <div class="flex flex-row items-center justify-center gap-2 w-full h-full pointer-events-none">
                            ${iconName ? `<i data-lucide="${iconName}" class="w-4 h-4 shrink-0 ${cnx ? 'text-red-400' : 'text-white'}"></i>` : ''}
                            <span class="truncate leading-none mt-[1px] ${cnx ? 'line-through' : ''}">${text}${cnxText}</span>
                            ${subText}
                        </div>
                    `;
                };
                ...
                if (currentBevs && currentBevs.length > 0) {
                    currentBevs.forEach((bev, idx) => {
                        if ((!bev.served && !bev.skipped) || bev.cancelled) {
                            const btn = document.createElement('div');
                            // [關鍵] 這裡的 class 確保了按鈕本體是 flex
                            btn.className = `flex flex-row items-center justify-center ${getServiceBtnClass(false, false, bev.cancelled)}`;
                            
                            const styleText = (bev.style && bev.style !== 'Normal') ? ` <span class="text-[10px] text-amber-200 ml-1 font-normal">(${bev.style})</span>` : '';
                            
                            // [關鍵] 內容再次包裹，確保圖示跟文字黏在一起置中
                            btn.innerHTML = `
                                <div class="flex flex-row items-center justify-center gap-2 pointer-events-none">
                                    ...
                                    <span class="truncate text-sm tracking-wide leading-none mt-[1px]">${bev.name}</span>
                                    ${styleText}
                                </div>
                            `;
